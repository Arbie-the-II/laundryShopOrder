...
<body>
    <div class="form-container">
        ...
        <?php if (isset($_GET['verification_sent'])): ?>
            <?php if ($_GET['verification_sent'] == '1'): ?>
                <div class="status-alert success">Verification email was sent to this customer's email address.</div>
            <?php else: ?>
                <div class="status-alert error">Failed to send verification email to this customer's email address.</div>
            <?php endif; ?>
        <?php endif; ?>
        <?php if (defined('APP_ENV') && APP_ENV === 'development'): ?>
            <?php if (!empty($customer['email_verification_token'])): ?>
                <div class="status-alert" style="background:#fff3cd;color:#856404;border:1px solid #ffeeba">Dev Link: <a href="<?= htmlspecialchars((isset($_SERVER['HTTPS']) ? 'https://' : 'http://') . $_SERVER['HTTP_HOST'] . '/order/verify_customer.php?token=' . $customer['email_verification_token']) ?>">Verify token link</a></div>
            <?php endif; ?>
        <?php endif; ?>
